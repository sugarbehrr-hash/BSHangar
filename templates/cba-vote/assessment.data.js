...
window.ASSESSMENT = {
  "renderContract": {
    ...
    "you_must": [
      ...
      "REGISTER FILTERING (collections): SHORT shows only entries whose register is 'web' or 'both'; FULL shows ALL. This governs marketCards[] (register 'web'|'pdf') and tables[] (register 'web'|'pdf'|'both'). Short must render only the ~4 web-tagged market cards, NOT all 11. Do not treat register as advisory.",
      "REGISTER PROSE: SHORT renders each item's `plain` and each chart's `plainCaption`; FULL renders `meaning`/`realterms` and `headline`. NEVER fall back from `plain` to `meaning` — if `plain` is missing that is a skill bug to report, not a reason to leak the technical register into the member view.",
      "COLOUR COMES FROM `polarity` ('pos'|'neu'|'neg'), which every chart, every scoreboard cell and every legend.verdicts entry carries. NEVER infer a colour by pattern-matching the verdict's words (e.g. testing for 'wash' or 'POSITIVE'). Reader-facing wording must be free to change without changing what a colour means; a verdict rename must never repaint a cell.",
