...
(function () {
  ...
  function makeFormatters(spec) {
    ...
    function tick(v) {
      if (fmt === "index") { var d = v - 100; return (d > 0 ? "+" : "") + pct(d); }
    ...
    function endVal(s) {
      ...
      if (fmt === "index") { var d = v - 100; return (d >= 0 ? "+" : "") + pct(d) + " since day one"; }
      if (fmt === "percent-of-start") { return (v / 100).toFixed(2) + "x your starting pay"; }
      if (fmt === "dollars") {
        var d0 = s.values[0];
  ...
  function applyUnit(spec, unitKey) {
    ...
    for (var i = 0; i < units.length; i++) {
      if (units[i].key === unitKey) { u = units[i]; break; }
    }
    if (!u) { for (var j = 0; j < units.length; j++) if (units[j]["default"]) u = units[j]; }
    if (!u) u = units[0];
    var out = JSON.parse(JSON.stringify(spec));
    if (typeof u.scaleBy === "number") {
      out.series.forEach(function (s) {
        s.values = s.values.map(function (v) { return Math.round(v * u.scaleBy * 100) / 100; });
      });
      if (out.axes && out.axes.y) { delete out.axes.y.min; delete out.axes.y.max; }
    }
    if (u.indexTo === "first") {
      out.series.forEach(function (s) {
        var base = s.values[0];
        if (base) s.values = s.values.map(function (v) { return Math.round(v / base * 1000) / 10; });
      });
      ...
      if (out.axes && out.axes.y) { out.axes.y.min = 100; delete out.axes.y.max; }
      if (out.shadeGap) { /* names unchanged, gap still valid */ }
    }
    out.axes = out.axes || {}; out.axes.y = out.axes.y || {};
  ...
  window.AssessmentChartUnits = function (spec) { return spec.units || []; };
  ...
  window.AssessmentChart = function (spec, opts) {
    ...
    var hasNames = valueLabels !== "none" && (spec.series || []).some(function (s) { return s.name; });
    var legRows = hasNames ? (spec.series || []).length : 0;
    var legendH = legRows ? legRows * 22 + 10 : 0;
    H = H + legendH;
    var ml = 78, mr = 22, mt = 22 + legendH, mb = 76 + annExtra + (rotateAllPre ? 26 : 0);
    var pw = W - ml - mr, ph = H - mt - mb;
    ...
    if (legendH) {
      ...
      seriesArr.forEach(function (s, si) {
        var c = colOf[si] || { col: "var(--navy-900)", dash: "" };
