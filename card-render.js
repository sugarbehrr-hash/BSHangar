...
(function () {
  ...
  // chart verdict → figure frame tint (cv-*)
  function chartVerdict(v) {
    if (/POSITIVE|WIN/i.test(v)) return { bg: "var(--success-100)", bd: "var(--success-600)" };
    if (/MIXED/i.test(v))        return { bg: "var(--cream-200)",   bd: "var(--gold-500)" };
    return { bg: "var(--red-100)", bd: "var(--red-600)" };
  }
  // category verdict → header pill colors (v-*)
  function verdictColors(v) {
    ...
    if (/Modest positive|positive/i.test(v)) return { bg: "var(--success-100)", fg: "var(--navy-900)" };
    if (/wash|neutral/i.test(v))            return { bg: "var(--cream-300)",   fg: "var(--ink-700)" };
  ...
  function inlineChartFigure(spec, ctx) {
    const cv = chartVerdict(spec.verdict || "");
    return '<figure style="margin:0 0 12px; background:var(--cream-100); border:1.5px solid var(--cream-300); border-radius:12px; padding:14px 10px 12px; break-inside:avoid;">'
      + '<p style="font-size:13px; line-height:1.55; margin:0 0 8px; color:var(--ink-700);">' + (spec.leadIn || "") + '</p>'
      + '<div style="display:inline-flex; align-items:center; padding:4px 11px; border-radius:999px; font-family:var(--font-heading); font-weight:800; font-size:10.5px; letter-spacing:0.03em; text-transform:uppercase; color:var(--navy-900); margin:0 0 10px; background:' + cv.bg + '; border:1.5px solid ' + cv.bd + ';">Verdict: ' + (spec.verdict || "") + '</div>'
      + chartHTML(spec, ctx)
  ...
  function chartFigure(spec, ctx) {
    const cv = chartVerdict(spec.verdict || "");
    return '<figure style="margin:0 0 16px; background:var(--white); border:1.5px solid var(--cream-300); border-radius:16px; padding:20px 22px 16px; break-inside:avoid;">'
  ...
  function topicBand(cat, ctx) {
    ...
    if (ctx.allGroups && cat.groups) {
      const rows = cat.groups.map(g => {
        const gv = verdictColors(g.verdict || "");
      ...
      const vc = verdictColors(cat.verdict || "");
  ...
  window.VoteCard = {
    ...
    normPlacement, formatStarts, chartHTML,
